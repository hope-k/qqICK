...
const useChat = (setChatLoading) => {
    ...
    const { API } = http();
    const { data: chats, mutate, } = useSWR(`/api/chat/index`, () => API.get('/api/chat/index').then(res => { setChatLoading(false); return res.data.chats }), { revalidate: true, revalidateOnFocus: true });
